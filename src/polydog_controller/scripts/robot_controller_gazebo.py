    ...
    try:
        ...
        for i in range(len(joint_angles)):
            ...
            if i == 0 or i == 1 or i == 6 or i == 7 :
                ...
                #print(command)
            #if i == 2 :
                #command = "#5P%d\r\n" % map1(-joint_angles[i]-joint_angles[i-1]-1.57)
                #serial_port.write(command.encode())
                #joint_angles1[10]=-joint_angles[i]
                #print(command)
            #if i == 3 :
                #command = "#20P%d\r\n" % map1(-joint_angles[i]-0.07535)
                #serial_port.write(command.encode())
                ...
                #print(command)
            #if i == 5 :
                #command = "#21P%d\r\n" % map1(joint_angles[i]+joint_angles[i-1]+1.57-0.05)
                #serial_port.write(command.encode())
                #joint_angles1[9]=-joint_angles[i]
                #print(command)
            #if i == 6 :
                #command = "#0P%d\r\n" % map1(joint_angles[i]+0.157+0.0471)
                #serial_port.write(command.encode())
                #joint_angles1[3]=joint_angles[i]
                #print(command)
            #if i == 7 :
                #command = "#2P%d\r\n" % map1(joint_angles[i]-2*0.157-0.07535)
                #serial_port.write(command.encode())
                ...
                #print(command)
            #if i == 10 :
                #command = "#17P%d\r\n" % map1(-joint_angles[i]+2*0.157+2*0.0785)
                #serial_port.write(command.encode())
